...
def allowed_double_pegs(cur_node: Node):
    ...
    second_move = allowed_move(cur_node.parent.state, cur_node.parent.parent.state)
    towers.sort() 
    second_move.sort()
    if towers == second_move:
        return towers
    else:
        return [item for item in towers if item not in second_move]
...
def generate_complete_config(state, allowed_pegs1):
    string_allow_pegs = [str(i) for i in allowed_pegs1]
...
def generate_complete_config_2_layer(state, allowed_pegs1):
    if len(allowed_pegs1) == 1:
        return [str(allowed_pegs1[0])*num_small_disk + state]
    elif len(allowed_pegs1) == 2:
        first = str(allowed_pegs1[0])*(num_small_disk-1)
        second = str(allowed_pegs1[1])
        return [first + second + state, second + first + state, str(allowed_pegs1[0])*num_small_disk + state,
                str(allowed_pegs1[1])*num_small_disk + state]
    else:  # len 4
        return [str(num_towers-1)*num_small_disk + state]


def generate_complete_configless(state, allowed_pegs1):
    if len(allowed_pegs1) == 2:
        first = str(allowed_pegs1[0])*(num_small_disk-1)
        second = str(allowed_pegs1[1])
        return [first + second + state, second + first + state, str(allowed_pegs1[0])*num_small_disk + state,
                str(allowed_pegs1[1])*num_small_disk + state]
