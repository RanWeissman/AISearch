    ...
    def solve(self):
        ...
        while len(frontier) > 0:
            ...
            if cur_node.state == self.initial_state:
                ...
                database[state_to_string(cur_node.state)] = (cur_node.g_n,
                                                             allowed_pegs(cur_node.state, cur_node.parent.state))
            
            explored.append(cur_node.state)

            # create list of children
            child_list = get_children(cur_node)

            for child in child_list:
                if (not in_list(child, frontier)) and (child.state not in explored):
                    frontier.append(child)
        
        return database


def get_children(cur_node):
    child_list = []
