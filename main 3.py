...
with open(f'Buckets/bucket_min_price_{num_disks-num_small_disk}.txt', 'w') as convert_file:
...
with open(f'Hanoi_PDB/hanoi_pdb_{num_disks-num_small_disk}.txt', "r") as file:
    file_data = file.read()
    database_big = json.loads(file_data)

with open(f'Buckets/bucket_min_price_{num_disks-num_small_disk}.txt', "r") as file:
    file_data = file.read()
    bucket_min_price = json.loads(file_data)
...
with open(f'PDBI_Heuristic/pdbi_hur_{num_disks-num_small_disk},{num_small_disk}.txt', 'w') as convert_file:
    convert_file.write(json.dumps(pdbi_hur))

# ###################### checking admissibility  ######################
with open(f'PDBI_Heuristic/pdbi_hur_{num_disks-num_small_disk},{num_small_disk}.txt', "r") as file:
    file_data = file.read()
    pdbi_hur = json.loads(file_data)

with open(f'Hanoi_PDB/hanoi_pdb_{num_disks}.txt', "r") as file:
    file_data = file.read()
    hanoi_pdb = json.loads(file_data)
